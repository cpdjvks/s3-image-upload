...
class FileUploadResource(Resource) :
    ...
    def post(self) :
        ...
        # S3에 업로드하면 된다.
        # S3에 업로드하기 위해서는 AWS에서 제공하는 파이썬 라이브러리인 boto3 라이브러리를 이용해야 한다.
        # boto3 라이브러리는 AWS의 모든 서비스를 파이썬 코드로 작성할 수 있는 라이브러리다.
        # 로컬에 이 라이브러리 설치한 적 없으므로 pip install boto3로 설치

        s3 = boto3.client('s3', aws_access_key_id = Config.AWS_ACCESS_KEY_ID,
                     aws_secret_access_key = Config.AWS_SECRET_ACCESS_KEY)
        
        try :
            s3.upload_fileobj(file, Config.S3_BUCKET, file.filename, ExtraArgs = {'ACL' : 'public-read',
